...
submit = (data) => {
    ...
    axios.get('http://localhost:8080/login',{
      ...
      return(
        response.data.success ? (document.getElementById('status').innerHTML = "Login Successful! You are being redirected to landing page in 5 seconds.",setTimeout(() => {window.location.replace('/landing?name='+ email)},5000)) : document.getElementById('status').innerHTML = response.data.message
      );
    });
  };
